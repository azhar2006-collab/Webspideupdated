...
// ── 0. CSS UPDATE FOR ALTERNATING CARDS ───────────────────────────────────
...
// B1 Cards (6 cards)
const b1CardsOldRegex = /<!-- B1 CARDS GRID -->[\s\S]*?<!-- B2 CARDS GRID -->[\s\S]*?<\/div>\s*<\/div>/;
...
// ── 4. SERVICES SECTION (WITH ALTERNATING REVERSE LAYOUT) ───────────────
html = html.replace(
  /<h2 class="section-title gradient-heading">.*?<\/h2>/i,
  '<h2 class="section-title gradient-heading">Our Front-End Development Services</h2>'
);

html = html.replace(
  /<p class="section-sub" style="margin-bottom:0">.*?<\/p>/i,
...
// 10 Service Cards (Odd = normal text left, Even = reverse text right / visual left)
...
console.log('Successfully updated indexfront1.html with alternating layout and full docx content! New size:', html.length);
